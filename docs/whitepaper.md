...
The first tier of the device layer is implemented today: peers on the same subnet discover each other and deliver messages directly, with every outbound message opportunistically dual-sent — offered to connected local peers alongside the guaranteed relay path — and deduplicated at the receiver. The consequence is that established conversations survive the loss of the relay, or of internet connectivity entirely, with no mode switch and no application code: on-site and air-gapped deployments are a supported configuration, not a special case. The proximity-radio and direct-internet tiers follow the same opportunistic-upgrade model and are specified, demand-gated, in the project's transport specification.
...
**Interoperability, when you want it.** Isolation is the default, but it is a *policy* at the identity layer, not a limit of the transport — because the relay genuinely cannot tell one app's packets from another's, the wall between two apps was never load-bearing. Two namespaces that both opt in can therefore message each other directly, end-to-end encrypted, across different apps and different operators at the same time. This is the email model: anyone able to reach anyone, the address is the protocol, no single party owns the network — except that here the relay can't read a word, and there is no domain to seize. It is deliberately *opt-in and key-based*: you reach people you have exchanged an invite or introduction with, not strangers typing a global handle. That last corner — a name that is human-meaningful, globally unique, and decentralised all at once — is provably unavailable without a central directory, and MeshWhisper declines to reintroduce one. The trade is explicit: registrar-free, un-seizable identity in exchange for stranger-by-handle discovery.

Routing is **DNS-free**. A relay reaches the relay that *homes* a recipient not by domain name but by key: the recipient's home-relay public key travels inside the self-describing invite, out of band, so there is no global directory mapping users to relays for anyone to seize or to mine. Relays resolve each other's keys to network endpoints through a small signed address book they gossip among themselves — public infrastructure addresses only, never anything about who talks to whom. A message is then forwarded straight to the single relay that homes its recipient rather than flooded across the federation, with flooding kept only as a fallback for contacts whose home relay isn't yet known. Where a relay sits behind NAT or can't be dialled directly, the packet rides existing federation links through a shared peer, onion-wrapped so each transit hop learns only the next hop — never the packet, and never its destination. The longer-term direction is the social-graph mesh the privacy section describes: paths that lengthen as adoption grows, so that no single relay sees both ends of a conversation.
...
There is a second path through the bootstrap problem, and it is more direct than the mesh's slow accretion: **interoperability inverts it.** When apps can opt into messaging one another, a developer who integrates the SDK no longer starts inside an empty network of only their own users — their users can reach everyone on the federated network from day one, the way a new email provider's customers can email the entire internet immediately rather than only each other. That turns the usual "nobody is here yet" cold start into "everyone is already reachable." It is the strongest answer the architecture has to the chicken-and-egg problem that an isolated island, by construction, can never escape.
...
**Identifier ownership.** Within a namespace, human-readable usernames are an optional layer above the cryptographic peer identity — apps can register a handle like `@alice` so contacts can be added without an out-of-band key exchange. The default ownership policy is sticky: once a key claims a handle, only that key can re-publish under it, and a different key attempting to take it over is rejected by the relay. Handover when needed — key rotation, device migration, gifting a handle — is via a short-lived authorization signed by the current owner's identity key, bound to a specific recipient key and an expiry, that the new key submits alongside its registration. The relay verifies the signature against the directory's record of the current owner before allowing the transfer. Apps that prefer the simpler "re-derive the same key from credentials = automatic re-claim" pattern (typical of password-derived identity flows) can opt the namespace into a last-writer-wins policy instead. The trade-off is explicit and per-namespace; the default exists because most apps want `@alice` to mean a coherent identity over time, which an unguarded last-writer-wins handle space cannot guarantee.

**Linked devices without a central device registry.** A user who wants the same account active on multiple devices simultaneously — phone, laptop, tablet, all receiving live — uses a linked-devices pattern in which each device has its own ratchet identity and the primary device signs a `device_added` announcement that fans out to existing contacts over the established session channels. Contacts maintain a local mapping from account key to device-key list; outbound messages are encrypted once per recipient device. The relay holds no device list, no notion of accounts, and no signal as to which devices belong to the same user — from its position, every device is an independent peer indistinguishable from any other. This is the Signal-style multi-device pattern with the central device registry stripped out, fitting the rest of the protocol's posture that the relay should hold as little structure about users as possible. The pattern co-exists with the simpler password-derived "same identity on every device" model: apps choose the shape that matches their identity story.
...
**Stage one** is bootstrap: the first relay node is live at `relay.meshwhisper.org` alongside Prudence — a reference PWA messaging application built entirely on the SDK, available at `prudence.meshwhisper.org`. MeshWhisper is simultaneously being integrated into existing production applications. A developer evaluating the SDK at this stage finds a working relay network with active users, not a proposal.
