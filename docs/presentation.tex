...
% ── Colour palette (matches whitepaper) ──────────────────────────────────────
\definecolor{mwblue}{HTML}{1A3A5C}
\definecolor{mwaccent}{HTML}{2D7DD2}
\definecolor{mwlight}{HTML}{EBF4FF}
...
\definecolor{nodebg}{HTML}{F0F7FF}
...
\setbeamertemplate{footline}{
  \begin{beamercolorbox}[wd=\paperwidth, ht=4mm, dp=1.5mm, leftskip=4mm, rightskip=4mm]{normal text}
    \color{mwgray}\tiny meshwhisper.net \hfill \insertframenumber\,/\,\inserttotalframenumber
  \end{beamercolorbox}
}
...
\author{GestureLoop Ltd · meshwhisper.net}
\date{April 2026}
...
\begin{center}
  ...
  {\color{mwlight}\large Messaging as Ambient Infrastructure}\\[16pt]
  \color{mwrule}\rule{60mm}{0.4pt}\\[10pt]
  {\color{mwgray}\small GestureLoop Ltd · meshwhisper.net · April 2026}
\end{center}
\end{frame}
}
...
\begin{columns}[T]
  \begin{column}{0.32\textwidth}
    \begin{block}{Stage 1 — Foundation \textcolor{mwgreen}{(now)}}
      \small GestureLoop deploys MeshWhisper in its own applications.\\[4pt]
      \textbf{meshwhisper.net} node live.\\
      \textbf{Prudence} reference PWA.\\
      Two live app integrations.\\[4pt]
      A developer evaluating the SDK finds a working relay network, not a proposal.
    \end{block}
  \end{column}
  \begin{column}{0.32\textwidth}
    \begin{block}{Stage 2 — Developer adoption}
      ...
      Foundation's share of relay capacity shrinks as developer infrastructure grows.
    \end{block}
  \end{column}
  \begin{column}{0.32\textwidth}
...
\begin{columns}[T]
  ...
  \begin{column}{0.40\textwidth}
    ...
    \textbf{meshwhisper.net}\\[2pt]
    \textcolor{mwaccent}{Prudence demo app}\\
    \texttt{prudence.meshwhisper.net}
    \end{mdframed}
  \end{column}
\end{columns}
\end{frame}
...
\begin{center}
  ...
  \color{mwrule}\rule{80mm}{0.4pt}\\[10pt]
  \begin{tabular}{ll}
    \color{mwlight}\textbf{SDK}     & \color{white}\texttt{npm install @meshwhisper/sdk} \\[4pt]
    \color{mwlight}\textbf{Node}    & \color{white}\texttt{docker run meshwhisper/node} \\[4pt]
    \color{mwlight}\textbf{Docs}    & \color{white}\texttt{meshwhisper.net} \\[4pt]
    \color{mwlight}\textbf{Demo}    & \color{white}\texttt{prudence.meshwhisper.net} \\[4pt]
    \color{mwlight}\textbf{Contact} & \color{white}\texttt{anton@gestureloop.com} \\
  \end{tabular}\\[14pt]
  \color{mwrule}\rule{80mm}{0.4pt}\\[8pt]
  {\color{mwgray}\small MIT licensed · Self-hosting always free · GestureLoop Ltd, Ireland}
\end{center}
\end{frame}
}
