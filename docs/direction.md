...
Ordering matters: **specification before implementation**. Adopters need to know what they're joining before they invest in launching a node. The `docs/federation.md` spec is a precondition for the implementation work, not a follow-up. Implementation lands when there's a second relay operator who wants to peer.
