...
`tests/cross-namespace.test.ts` now proves automatic **bidirectional** A↔B messaging when both opt in, plus default isolation.

**Stage-2 — cross-operator routing, proven (2026-06-15).** The full "email model" end to end: two SDK peers in *different namespaces* homed on *different operators'* relays (federated), exchanging a real E2EE message that crosses **both** boundaries at once. The notable result: it needed **zero new SDK or relay code** — federation v1 already forwards opaque packets between relays (`forwardFromLocal` + `classifyLocal`; see `tests/federation.test.ts`), and interop already addresses into the peer's namespace, so the two compose. The SDK is relay-agnostic: it sends to its own node, and the relays route. `tests/cross-operator.test.ts` proves it with two federated relays + two live SDK worker processes (`tests/helpers/interop-worker.mts`) — different app, different operator, delivered + decrypted. Remaining (the genuinely hard part): DNS-free relay *location* — today a relay forwards a packet to a peer it can deliver/store for (connected client or push-registered, homed-but-offline), but discovering *which* operator homes an arbitrary destHash without a flood needs mesh/DHT rendezvous.
