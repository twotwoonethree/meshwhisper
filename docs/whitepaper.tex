...
\hypersetup{
  ...
  pdfauthor={MeshWhisper Foundation}
}
...
\fancyfoot[R]{\small\color{mwgray}MeshWhisper Foundation}
...
\begin{titlepage}
  ...
  \color{white}
  \vspace*{40mm}
  {\fontsize{28}{34}\selectfont\bfseries MeshWhisper\par}
  \vspace{6pt}
  {\fontsize{18}{24}\selectfont Messaging as Ambient Infrastructure\par}
  \vspace{20pt}
  \color{mwaccent}
  \rule{60mm}{1pt}
  \vspace{20pt}
  \color{white!80!mwblue}
  {\normalsize MeshWhisper Foundation\quad·\quad April 2026\par}
  \vfill
  {\small\color{white!60!mwblue}
...
\textbf{Stage one} is foundation: GestureLoop deploys MeshWhisper in its own applications, creating real running infrastructure with real users. The first Foundation node launches at \texttt{meshwhisper.net} alongside Prudence — a reference PWA messaging application built entirely on the SDK. MeshWhisper is simultaneously being integrated into two existing live applications. A developer evaluating the SDK in stage one finds a working relay network with active users, not a proposal.
...
Foundation node: \texttt{meshwhisper.net}\quad·\quad Reference app: Prudence (\texttt{prudence.meshwhisper.net})

\vspace{6pt}
MeshWhisper Foundation, Ireland\quad·\quad Commercial services by GestureLoop Ltd\quad·\quad \texttt{anton@gestureloop.com}
}
