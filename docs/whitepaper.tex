...
\begin{titlepage}
  ...
  {\small\color{white!65!mwblue}
    ...
    \texttt{anton@meshwhisper.org}\par
  }
\end{titlepage}
...
% Relay in the centre
\node[relay, align=center] (relay) {Relay\\Node};
...
% Annotations
\node[font=\footnotesize, text=mwgray, align=center, below=6mm of p1]
  {outbound WebSocket\\(phones on cellular)};
\node[font=\footnotesize, text=mwgreen, align=center, below=6mm of l1]
...
\node[font=\small\bfseries, text=mwblue, above=5mm of nodeL] {Low Density};
...
\node[font=\footnotesize, text=mwgray] at (0, -2.5) {Privacy level:};
\fill[mwred!80] (-1.4, -2.95) rectangle (1.4, -2.7);
\node[font=\footnotesize\bfseries, text=white] at (0, -2.825) {WEAK};
...
\node[font=\small\bfseries, text=mwblue, above=5mm of nodeB] {High Density};
...
\node[font=\footnotesize, text=mwgray] at (0, -2.5) {Privacy level:};
\fill[mwgreen!80] (-1.4, -2.95) rectangle (1.4, -2.7);
\node[font=\footnotesize\bfseries, text=white] at (0, -2.825) {STRONG};
...
\textbf{Identifier ownership.} Within a namespace, human-readable usernames are an optional layer above the cryptographic peer identity --- apps can register a handle like \texttt{@alice} so contacts can be added without an out-of-band key exchange. The default ownership policy is sticky: once a key claims a handle, only that key can re-publish under it, and a different key attempting to take it over is rejected by the relay. Handover when needed --- key rotation, device migration, gifting a handle --- is via a short-lived authorization signed by the current owner's identity key, bound to a specific recipient key and an expiry, that the new key submits alongside its registration. The relay verifies the signature against the directory's record of the current owner before allowing the transfer. Apps that prefer the simpler ``re-derive the same key from credentials = automatic re-claim'' pattern (typical of password-derived identity flows) can opt the namespace into a last-writer-wins policy instead. The trade-off is explicit and per-namespace; the default exists because most apps want \texttt{@alice} to mean a coherent identity over time, which an unguarded last-writer-wins handle space cannot guarantee.

\textbf{Linked devices without a central device registry.} A user who wants the same account active on multiple devices simultaneously --- phone, laptop, tablet, all receiving live --- uses a linked-devices pattern in which each device has its own ratchet identity and the primary device signs a \texttt{device\_added} announcement that fans out to existing contacts over the established session channels. Contacts maintain a local mapping from account key to device-key list; outbound messages are encrypted once per recipient device. The relay holds no device list, no notion of accounts, and no signal as to which devices belong to the same user --- from its position, every device is an independent peer indistinguishable from any other. This is the Signal-style multi-device pattern with the central device registry stripped out, fitting the rest of the protocol's posture that the relay should hold as little structure about users as possible. The pattern co-exists with the simpler password-derived ``same identity on every device'' model: apps choose the shape that matches their identity story.
...
\textbf{Stage one} is bootstrap: the first relay node is live at \texttt{relay.meshwhisper.org} alongside Prudence --- a reference PWA messaging application built entirely on the SDK, available at \texttt{prudence.meshwhisper.org}. MeshWhisper is simultaneously being integrated into existing production applications. A developer evaluating the SDK at this stage finds a working relay network with active users, not a proposal.
