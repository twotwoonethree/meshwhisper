...
3. **Internet — WebSocket over HTTPS (port 443)** — The primary transport for most messages. Every NAT, firewall, carrier network, and corporate proxy on earth passes HTTPS on port 443. No NAT traversal, no hole-punching, no STUN, no TURN. The protocol uses the front door, not a side window. Devices that can accept inbound connections (laptops on broadband, desktops, dedicated home relay nodes, Foundation relay nodes) advertise this capability. Devices that can't (most phones on cellular) maintain outbound WebSocket connections to connectable peers — defaulting to Foundation relay nodes when no connectable social-graph peer is available.
...
**Non-connectable devices** (most phones on cellular) maintain persistent outbound WebSocket connections to connectable peers. When the social graph contains no connectable device — the common case for mobile-only consumer apps at early adoption — devices connect outbound to the nearest available Foundation Relay Node (see §5.6). Both phones maintain outbound connections to the relay; the relay bridges them. This mirrors how WhatsApp works, but the "server" is operated as public infrastructure rather than by the app vendor.

**Backbone vs. consumers:** Phones on cellular are principally consumers of the mesh. The backbone is connectable devices: laptops on broadband, desktops, Foundation Relay Nodes, and Phase 4 home relay hardware. Phone relay activity is a bonus when the app is foregrounded, not a dependency.

### 5.6. Foundation Relay Network

Most consumer apps — fitness trackers, marketplaces, coaching tools — are mobile-only. Their users have no desktop companion. The organic social graph of such apps contains no connectable devices. The Foundation Relay Network fills this gap.

The MeshWhisper Foundation operates a small number of always-on relay nodes as public infrastructure. These are connectable bridge points that allow two non-connectable phones to exchange messages. They are not servers in the traditional sense: they hold no user data, cannot read message content, and do not authenticate or identify users.
...
- Nodes hold encrypted blobs temporarily for store-and-forward delivery
- Nodes are stateless and interchangeable — any node can be replaced by any other
- The SDK ships with hardcoded Foundation relay endpoints and a discovery mechanism for locating alternative or additional relay operators

**Trust model:**

Foundation relay nodes occupy the same position in the threat model as an ISP carrying encrypted traffic. A relay node can observe:
- That a connection exists (source IP, relay IP, timing)
...
If all Foundation relay nodes are unreachable, the protocol continues to function between devices that can reach each other directly or through organic connectable peers in their social graphs. Foundation relays are accelerators, not dependencies. A device that was previously routing through a Foundation relay will queue messages and retry through alternative paths. The degradation is graceful.

**Honest framing:**

"Serverless for the developer" is accurate. "Serverless, full stop" is not. The Foundation operates relay infrastructure as a public good, funded through enterprise licensing and commercial support tiers. As adoption grows and more connectable devices join the mesh — particularly Phase 4 home relay nodes — dependence on Foundation infrastructure decreases organically. This is the BitTorrent tracker model: the trackers don't carry the content, and the network functions without them once sufficient peers exist. Unlike BitTorrent, the Foundation relays are openly documented as part of the architecture rather than treated as a temporary embarrassment.

**Developer control:**

Developers can override the default Foundation relay endpoints, add additional relay operators, or self-host relay infrastructure. The relay node implementation is open source. Enterprise customers can run private relay fleets with no traffic transiting Foundation nodes.
...
**Prekey directory service for stranger-to-stranger contact:** Apps where users need to contact people they haven't met — marketplaces, social discovery, coaching platforms — require a different approach. The MeshWhisper Foundation operates a lightweight **prekey directory service** alongside the relay network. This service:
...
The app registers a user's prekey bundle with the directory on first launch. When a buyer in a marketplace app initiates contact with a seller for the first time, the SDK fetches the seller's prekey bundle from the directory, initiates X3DH, and the conversation proceeds peer-to-peer from that point. The directory is consulted once per new contact; all subsequent communication is direct and the directory is not involved.

Developers can use the Foundation-hosted directory, self-host their own, or federate between operators. The directory holds only public key material — there is no private information to protect. Enterprise customers can run a fully private directory with no dependency on Foundation infrastructure.
...
1. The sending device identifies the **nearest available relay** — a device that is socially proximate to the recipient, currently online, and has a positive reciprocity balance. Foundation Relay Nodes are eligible relays and are preferred when no socially proximate relay is available.
...
- **Server compromise:** No application servers exist to compromise. Foundation relay nodes hold only opaque encrypted blobs with no user-identifying data.
- **Legal compulsion of relay infrastructure:** Foundation relay nodes cannot produce plaintext or user identity under legal compulsion — they do not possess it. They can produce connection metadata (source IPs, timing, volumes), equivalent to what an ISP produces.
...
- **Foundation relay metadata:** When messages transit Foundation relay nodes, the Foundation can observe connection metadata (source IPs, timing, destination hashes). This is equivalent to ISP-level metadata and weaker than direct device-to-device communication. Users who require stronger metadata protection should use a VPN. As organic mesh density grows, fewer messages need to transit Foundation relays.
...
- **Deploy Foundation Relay Network** — initial 5 nodes across Europe, US, Asia-Pacific
- **Deploy prekey directory service** — Foundation-hosted, namespace-scoped
- Publish spec and SDK as open source

**Deliverables:** Protocol spec v1.0, Android SDK, iOS SDK, TypeScript SDK (web), demo app, Foundation relay network (operational).
...
| :--- | :--- |
| **Adoption chicken-and-egg** | Foundation Relay Network provides reliable delivery from day one for any mobile-only user base. The SDK has standalone value before organic mesh density develops. |
...
| **Key discovery without servers** | Addressed by the prekey directory service (§6.2) for stranger-to-stranger contact. Gossip-based distribution handles existing social graphs. Developers can self-host the directory or use Foundation infrastructure. |
| **Connectable device density** | Addressed by the Foundation Relay Network (§5.6), which provides reliable phone-to-phone bridging from launch regardless of organic connectable device count. Foundation relay dependence decreases as mesh density grows. |
| **iOS background relay** | The SDK does not depend on iOS background relay. Phones relay when foregrounded; BGTaskScheduler enables brief sync bursts (30s) for message delivery. The relay backbone is connectable devices (laptops, Foundation nodes, Phase 4 hardware), not phones. |
...
| **Foundation relay governance** | The Foundation Relay Network requires a governance framework: which legal jurisdictions the nodes operate in, what the policy is if a government demands connection metadata, and how relay operators beyond the Foundation are vetted. This should be addressed in the foundation charter (Phase 3). |
